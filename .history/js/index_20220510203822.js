...
$(document).ready(function() {
        ...
        try {
            ...
            if ($(ad).length == 0 || type) {
                ...
                app.getSonglists(id).then(song => {
                    ...
                    singerDetailStr(song.singerID, '.baike-1')
    ...
    $('body').click(function(e) {
        if ($(e.target).attr('class') != 'notlogin' || !$(e.target).attr('class') != 'header') {
...
$('.userRec').on('click', function() {
    ...
    $('.main_cont tbody').empty();
    $('.main_cont tbody').attr('type', '')
    $('iframe').attr('src', 'https://mu-two.vercel.app/recommend/songs')
    var _id = $(this).attr('data-id');
    new Promise((v, f) => {
        $('.main_cont tbody').append(load2)
        $('.main_cont tbody').attr('type', 'song')

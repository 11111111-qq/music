...
function commentStr(comment) {
    ...
    span12 = $('<span></span>').html(comment.content).attr('title', (comment.content) p1.append(span11, span12);
            var p2 = $('<p></p>').addClass('tim'); span21 = $('<span class="float-start"></span>').html(comment.time); span22 = $('<span class="float-end px-1"> | <span class="back"> 回复 </span> </span >').prepend($('<i class="iconfont icon-good text-primary"></i>').html(comment.like)); p2.append(span21, span22); li.append(img, p1, p2);
            return li;
        }
        // songComment(186016)

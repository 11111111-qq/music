...
$('#loaddata').load('https://mu-two.vercel.app/login/refresh', e => {
    ...
    if (JSON.parse(e).msg == '需要登录') {
        var user = getCookie('tingmusic');
        if (!user) {
            alert('请重新登录！')
        } else {
            var ee = '?' + 'phone=' + user.phone + '&' + 'password=' + user.password;
            $('#loaddata').load('https://mu-two.vercel.app/login/cellphone' + ee,
                function(e) {
                    $('#loaddata').load('https://mu-two.vercel.app/login/refresh', g => {
                        console.log()
                    })
                })
        }

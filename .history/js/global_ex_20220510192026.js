...
function singerDetailStr(id, ele) {
    ...
    app.getSingerDetail(id).then(e => {
        ...
        if (e.name) {
            var cont = $(ele);
            var img = $('<img class="singerPic">').attr('src', e.pic);
            var h4 = $('<h4></h4>').html(e.name);
            var span1 = $('<span></span>').html(e.transNames);
            var rank = e.rank.rank ? e.rank.rank : "--";
            var aa = $('<a></a>').html('排名：' + rank)
            var p = $('<p></p>').append($("<span></span>").html('<专辑：' + e.albumSize + '>'));
            p = p.append($("<span></span>").html('<音乐：' + e.musicSize + '>'));
            p = p.append($("<span></span>").html('<mv：' + e.mvSize + '>'));
            var pp2 = $('<div class="title"></div>').append(img, h4, span1, aa, p)
            var pp = $('<p class="cont" style="text-align: start;"></p>').html(e.briefDesc);
            // console.log('a', pp2.get(0));
            cont.append(pp2, pp);
        } else {
            cont.append($('<p>wyy暂未收录该艺人！</p>'))
        }
